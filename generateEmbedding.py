...
def generate_embedding(df: pd.DataFrame, model_path, text_cols, value_cols, category_cols, label_cols: str, label_map=None) -> None:
    ...
    if "mask" in df.columns:
        for i in range(6):
            mask.append(df["mask"][i])
            print(mask)
    return X, y, mask, label_map, reverse_label_map